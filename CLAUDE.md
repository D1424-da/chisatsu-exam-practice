...
Study-mode filters (`filter-mode` select) each have a dedicated scoring/filter function in `app.js`: `weakScore` (苦手優先), `reviewPriorityScore`/`isDueForReview` (復習優先, uses `getRecord` — does **not** aggregate inline-OX sub-records, a known limitation), `priorityReviewScore` (優先復習, uses `getEffectiveRecord` and correctly aggregates), `isPerfectLimb`/`isAmbiguousLimb` (完璧/あいまい), `computeFewAnswersInfo` (回答数が少ないもの). The last one uses `max(1, median answer count)` over the currently-filtered limbs as its cutoff, and selects limbs strictly below it. **Do not switch this to a fraction of the mean** — that was tried and broke exactly where the mode matters most: with counts spread over 1–3 (mean 2) the cutoff collapsed to "below 1", so limbs answered once were excluded and the mode returned nothing. The median tracks the actual distribution instead, so the bottom half is always selectable. Properties worth keeping: the category empties out on its own when every limb has the same count (nothing is relatively behind), the `max(1, …)` floor keeps unanswered limbs selectable when the median is 0, and the count-bar button prints the live cutoff (`回答数が少ない: 20（4回未満）`) since it shifts as study progresses. `priorityReviewScore` additionally adds a smoothly-decaying `4 / (total + 1)` bonus so limbs answered only a few times get surfaced ahead of heavily-drilled ones, without overriding the stronger unanswered (`total === 0`) bonus.
