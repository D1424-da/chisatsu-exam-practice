...
- **Inline ○× questions**: a limb whose `text` embeds multiple parenthesized sub-judgments (e.g. `（①…）〇×`), parsed by `parseInlineOxItems`/`extractInlineOxKey`. Each sub-item is answered and scored independently but rolls up into the parent limb's stats via `getEffectiveRecord(limb)`, which aggregates sub-record ids of the form `` `${limbId}::${key}` `` (see `makeInlineRecordId`). Most stats/filtering code has to special-case this aggregation — when touching mastery/scoring logic, check whether it reads `getRecord(limb.id)` directly (wrong for inline-OX) or `getEffectiveRecord(limb)` (correct).
