...
Study-mode filters (`filter-mode` select) each have a dedicated scoring/filter function in `app.js`: `weakScore` (苦手優先), `reviewPriorityScore`/`isDueForReview` (復習優先, uses `getRecord` — does **not** aggregate inline-OX sub-records, a known limitation), `priorityReviewScore` (優先復習, uses `getEffectiveRecord` and correctly aggregates), `isPerfectLimb`/`isAmbiguousLimb` (完璧/あいまい), `computeFewAnswersInfo` (回答数が少ないもの). The last one groups limbs into tiers by answer count and walks tiers from the lowest, accumulating until it has ~`FEW_ANSWERS_TARGET` limbs; the returned `cutoff` is inclusive (`count <= cutoff`), and `-1` means nothing qualifies. **Do not reduce this to a single statistic of the distribution** — both a fraction of the mean and the median itself were tried and each collapsed to "unanswered only" whenever the low end of the distribution *is* the representative value. With 1回×600 / 2回×300 / 3回×100 the median is 1, so "below the median" means "below 1", and the 600 limbs that are plainly behind get excluded — the mode returns nothing. Walking the actual tiers cannot degenerate that way. Properties worth keeping: the highest tier is never included (being the most-practiced is not "behind"), all-equal counts yield `cutoff = -1` (nothing is relatively behind) *except* when every limb is unanswered, where all qualify, and the count-bar button prints the live cutoff (`回答数が少ない: 600（1回以下）`) since it shifts as study progresses. `priorityReviewScore` additionally adds a smoothly-decaying `4 / (total + 1)` bonus so limbs answered only a few times get surfaced ahead of heavily-drilled ones, without overriding the stronger unanswered (`total === 0`) bonus.

One `filter-mode` value, `exam` (本番モード), is structurally different from the rest: it does not go through `getLimbsMatchingFilters()`/per-limb scoring at all. `startSession` special-cases `mode === 'exam'` and calls `startExamSession`, which builds `session.queue` out of whole **question** objects (via `getExamEligibleQuestions`) instead of individual limbs, and sets `session.examMode = true`. `renderCurrentLimb` branches on that flag to call `renderExamQuestion` instead of the normal single-limb renderer, showing every limb of the question together (ア〜オ, like the `manage` page's question cards) and withholding grading until all limbs are answered, per-question, mimicking the real combination-question exam format. Two things make this safe to bolt onto the existing session machinery without touching it: `session.resumeEligible = false` makes `saveStudySessionSnapshot` a no-op (its first line checks that flag), so the exam queue's question ids never leak into the resume-snapshot format that assumes limb ids; and grading in this mode never calls `addRecord`/`queueRecordFieldsSync` — pass/fail is computed locally in `renderExamQuestion` against each limb's `correct` flag and discarded, by explicit user request, so it never touches `records`, mastery, spaced-repetition state, or weak-limb lists. `getExamEligibleQuestions` only admits questions where every limb is plain ○×  (`isPlainOxLimb` excludes choice/text-answer/inline-OX limbs) — those other limb types have their own answer UI that doesn't fit a "mark all limbs then grade" screen. Note the real exam's original 5-choice numbered options (e.g. "1 アイ 2 アウ 3 イエ…") are **not** in the scraped data — only a `correctCombo` field with the single correct combination survives, and even that is unused by this mode; a whole exam question is graded correct only when every one of its limbs' individual ○/× judgments matches, which is a reasonable but not identical stand-in for "picked the right numbered option."
