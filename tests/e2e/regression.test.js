...
test.describe('回帰テスト: 全自動改善対応の確認', () => {
  ...
  test('FEATURE: 苦手肢リストを問題ごとにまとめられ、未克服肢数が多い問題が優先表示される', async ({ page }) => {
    const result = await page.evaluate(() => {
      if (typeof renderStats !== 'function') return null;
      const origGetAuthUid = window.getAuthUid;
      window.getAuthUid = () => 'test-uid';
      ...
      // q1: 3肢中2肢が未克服（濃度が高い問題）。q2: 1肢だけ未克服だが誤答回数は多い。
      records = {};
      questions = [
        { id: 'q1', subject: 'S', questionText: '問1', limbs: [
          { id: 'q1a', text: 'ア', correct: true, explanation: '' },
          { id: 'q1b', text: 'イ', correct: false, explanation: '' },
          { id: 'q1c', text: 'ウ', correct: true, explanation: '' },
        ] },
        { id: 'q2', subject: 'S', questionText: '問2', limbs: [
          { id: 'q2a', text: 'エ', correct: true, explanation: '' },
        ] },
      ];
      records['q1a'] = mk(0, 1);
      records['q1b'] = mk(0, 1);
      records['q1c'] = mk(3, 0); // 克服済み・弱点対象外
      records['q2a'] = mk(0, 9); // 誤答回数は多いが1問1肢のみ

      document.getElementById('weak-hide-high-rate').checked = false;
      document.getElementById('weak-list-limit').value = '0';
      document.getElementById('weak-group-mode').value = 'question';
      renderStats();

      const groups = [...document.querySelectorAll('.weak-question-group')];
      const firstGroupMeta = groups[0]?.querySelector('.weak-question-group-meta')?.textContent || '';
      const firstGroupLimbRows = groups[0]?.querySelectorAll('.weak-limb-row').length ?? 0;
      const totalText = document.getElementById('weak-limbs-total').textContent;
      const sortDisabled = document.getElementById('weak-list-sort').disabled;

      window.getAuthUid = origGetAuthUid;
      return { groupCount: groups.length, firstGroupMeta, firstGroupLimbRows, totalText, sortDisabled };
    });
    if (result === null) test.skip();

    // 2問（q1, q2）がグループとして表示される
    expect(result.groupCount).toBe(2);
    // 未克服肢数が多い q1（2/3肢）が q2（1/1肢）より先に来る
    expect(result.firstGroupMeta).toContain('未克服 2/3肢');
    expect(result.firstGroupLimbRows).toBe(2);
    // 件数表示は「問」単位、かっこ内に肢の実数も出す
    expect(result.totalText).toContain('2 問');
    expect(result.totalText).toContain('肢 3 件');
    // グループ表示中は肢単位の並び順セレクトを無効化する
    expect(result.sortDisabled).toBe(true);
  });

  test('FEATURE: 苦手肢リストを論点（キーワード）ごとにまとめられ、未分類は末尾に回る', async ({ page }) => {
    const result = await page.evaluate(() => {
      if (typeof renderStats !== 'function' || typeof classifyWeakTopic !== 'function') return null;
      const origGetAuthUid = window.getAuthUid;
      window.getAuthUid = () => 'test-uid';
      const mk = (c, w) => ({
      ...
      records = {};
      questions = [
        // 「分筆」論点が2問にまたがって存在する（＝問題ごとでは見えない横断的な弱点）
        { id: 'q1', subject: 'S', questionText: '分筆の登記に関する記述', limbs: [
          { id: 'q1a', text: '分筆の登記の申請をすることができる。', correct: true, explanation: '' },
        ] },
        { id: 'q2', subject: 'S', questionText: '土地の合筆に関する記述', limbs: [
          { id: 'q2a', text: '合筆の登記の申請をすることができる。', correct: true, explanation: '' },
        ] },
        // どのキーワードにも掛からない問題
        { id: 'q3', subject: 'S', questionText: '謎の手続に関する記述', limbs: [
          { id: 'q3a', text: '謎の手続をすることができる。', correct: true, explanation: '' },
        ] },
      ];
      records['q1a'] = mk(0, 3);
      records['q2a'] = mk(0, 1);
      ...
      document.getElementById('weak-hide-high-rate').checked = false;
      document.getElementById('weak-list-limit').value = '0';
      document.getElementById('weak-group-mode').value = 'topic';
      renderStats();

      const groups = [...document.querySelectorAll('.weak-question-group')];
      const names = groups.map(g => g.querySelector('.weak-topic-name')?.textContent || '');
      const firstGroupLimbRows = groups[0]?.querySelectorAll('.weak-limb-row').length ?? 0;
      const totalText = document.getElementById('weak-limbs-total').textContent;

      window.getAuthUid = origGetAuthUid;
      return { names, firstGroupLimbRows, totalText };
    });
    if (result === null) test.skip();

    // q1(分筆)とq2(合筆)は同じ「分筆・合筆」論点にまとめられ2肢になる
    expect(result.names[0]).toContain('分筆・合筆');
    expect(result.firstGroupLimbRows).toBe(2);
    // どのキーワードにも掛からないq3は「その他（未分類）」として末尾に来る
    expect(result.names[result.names.length - 1]).toBe('その他（未分類）');
    expect(result.totalText).toContain('論点');
  });
